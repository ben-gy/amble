...
describe('public/manifest.webmanifest', () => {
  ...
  it('names the game, so the home-screen label is not a URL', () => {
    m = JSON.parse(RAW) as Manifest;
    expect(m.name).toBe('Amble');
    expect(m.short_name).toBe('Amble');
  });
...
describe('the deployed origin', () => {
  it('CNAME points at this game\'s own subdomain', () => {
    // A stale CNAME copied from the game this one was scaffolded off would take
    // over the OTHER game's domain the first time it deployed.
    expect(read('public/CNAME').trim()).toBe('amble.benrichardson.dev');
  });

  it('robots.txt and sitemap.xml agree with it', () => {
    expect(read('public/robots.txt')).toContain('https://amble.benrichardson.dev/sitemap.xml');
    expect(read('public/sitemap.xml')).toContain('https://amble.benrichardson.dev/');
  });
});
...
describe('index.html install metadata', () => {
  ...
  it('loads exactly two external scripts — the beacon and the feedback widget', () => {
    // The privacy promise depends on this: the analytics beacon and the hosted
    // feedback widget are the ONLY third-party scripts allowed, and both must be
    // present (the widget is required, not merely permitted).
    const scripts = [...HTML.matchAll(/<script[^>]*src="([^"]+)"/g)].map((m2) => m2[1]);
