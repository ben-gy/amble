 ...
 *  - DISAGREEING ABOUT THE MODE. Rootbound's modes change the shape of the shared
 *    colony and what counts as a closed ring. Two peers reading their own local
 *    mode picker are playing two different games with no error anywhere.
...
/** Rootbound's round settings — the thing that must travel frozen. */
interface Opts {
  mode: string;
}
...
describe('both peers vote, exactly one round starts', () => {
  it('same round number, same seed, same frozen roster, one host', () => {
    const a = seat('a', 'longroad'); // the host's pick
    const b = seat('b', 'towpath'); // the guest's pick — and it must NOT win
    bus.flush();
  ...
  it("the HOST's mode travels frozen, and the guest's local pick is ignored", () => {
    const a = seat('a', 'longroad');
    const b = seat('b', 'towpath');
    bus.flush();
    tick(5000);
    ...
    expect(a.got[0].opts).toEqual({ mode: 'longroad' });
    expect(
      b.got[0].opts,
      'the guest played its own mode — a different colony to the host',
    ).toEqual({ mode: 'longroad' });
  ...
  it('the guest can see what the host has chosen before the round starts', () => {
    ...
    expect(a.rounds.state().hostOpts).toEqual({ mode: 'longroad' });
  });
});
...
describe('rounds are monotonic', () => {
  ...
  it('a duplicate start for the round already playing is a no-op', () => {
    const a = seat('a', 'wayward');
    const b = seat('b', 'wayward');
    bus.flush();
    tick(5000);
    playRound(a, b);
