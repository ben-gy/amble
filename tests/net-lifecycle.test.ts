/**
 * net-lifecycle.test.ts — live-P2P contract gate: ONE ROOM PER SESSION, and the
 * most valuable trivial test in the suite. A rematch versions rounds INSIDE the
 * living room; leaving and rejoining to reset hands back a dying room object and
 * every peer then elects itself host. The engine makes the trap throw; this pins
 * that it does, plus the one-join invariant. No transport required.
 */
...
const CFG = { appId: 'amble@1', roomId: 'K7QM' };
...
describe('a net that was never left still holds its slot', () => {
  ...
  it('reports itself as active so a stray second createNet is caught', () => {
    resetNetStats();
    createNet(CFG);
    expect(netStats().active).toEqual(['amble@1/K7QM']);
  });
});
