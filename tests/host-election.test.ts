 ...
 *
 * Rootbound resolves each turn per-peer, so it needs a host for exactly one
 * thing: deciding who plays the next round and freezing its settings. That makes
 * host election look unimportant right up to the moment it goes wrong, at which
 * point the room has two hosts, or none, and neither state is visible to a
 * player — the lobby just never starts.
 *
...
async function spawn(id: string, claimHost = false): Promise<Net> {
  ...
  return createNet({ appId: 'amble@1', roomId: 'ROOM', claimHost });
}
